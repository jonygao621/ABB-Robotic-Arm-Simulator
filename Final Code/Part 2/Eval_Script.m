...
for i=1:length(test_case)
    ...
    if (abs(sum(output.percentError)) > 0.0001)
        nIncorrect = nIncorrect + 1;
        fprintf('---ERROR---ERROR---ERROR---ERROR---ERROR---ERROR---ERROR---ERROR---\n\n');
        inputs = struct2cell(fun.Inputs);
        for k = 1:length(inputs)
            fprintf('Input %2.0f:\n',k);
            val = inputs{k};
            if (isstruct(val) == true)
                names = fieldnames(val);
                vals = struct2cell(val);
                fprintf('Struct of length: %2.0f\n', length(names));
        ...
        outputs = struct2cell(fun.Outputs);
        for k = 1:length(outputs)
            fprintf('Desired Output %2.0f:\n',k);
            val = outputs{k};
            if (isstruct(val) == true)
                names = fieldnames(val);
                vals = struct2cell(val);
